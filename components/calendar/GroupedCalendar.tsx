...
function parseFlatOccurrences(payload: unknown) {
  ...
  for (const item of payload) {
    ...
    parsed.push({
      ...
      facilitator: asRecord(record?.facilitator)
        ? {
            id: toPositiveInt(asRecord(record?.facilitator)?.id ?? null) ?? undefined,
            display_name: pickString(asRecord(record?.facilitator), ["display_name", "displayName", "name"]) || undefined,
            photo_url: pickString(asRecord(record?.facilitator), ["photo_url", "photoUrl", "image_url", "imageUrl"]) || undefined,
            has_public_profile:
              typeof asRecord(record?.facilitator)?.has_public_profile === "boolean"
                ? (asRecord(record?.facilitator)?.has_public_profile as boolean)
                : undefined,
          }
        : null,
